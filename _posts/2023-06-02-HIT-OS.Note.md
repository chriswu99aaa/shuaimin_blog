...
用户启动计算机，就是启动了一系列的进程。用户管理计算机就是管理这一堆进程
...
```
switch 函数是进程管理的核心。它的参数就是PCB。每个进程有一个PCB，通过管理PCB 管理进程

#### 交替的三个部分 队列操作+调度+切换

#### 多进程图像：多进程如何影响

* 多个进程同时在存在于内存会出现两个进程代码互相可以访问的问题
* 解决办法：限制对地址的读写
...
![image](../pictures/two-stacks.png)

![image](../pictures/web-explore.png)
...
![image](../pictures/multi-thread.png)
...
![image](../pictures/kernel-stack.png)

从用户级线程切换到核心级线程
![image](../pictures/switch-to.png)
